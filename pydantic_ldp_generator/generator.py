...
class TemplateGenerator:
    """Generates Databricks Lakeflow Declarative Pipeline templates from pydantic models using the new dp library."""
    ...
    def generate_pipeline(self, 
        ...
        """
        Generate complete Lakeflow Declarative Pipeline templates using the new dp library.
        ...
        """
        self.logger.info(f"Generating Lakeflow pipelines for {len(schemas)} models")
        ...
        # Generate reusable validation rules module (Databricks best practice)
        rules_module = self.generate_rules_module(schemas)
        generated_files["validation_rules.py"] = rules_module
...
# {{ schema.model_name }} validation rules as reusable expectations
def get_{{ schema.model_name|snake_case }}_rules():
    """
    Returns validation rules for {{ schema.model_name }} as a dictionary.
    
    This pattern follows Databricks best practices for portable and reusable expectations.
    Rules can be easily maintained, audited, and applied to multiple datasets.
    
    Generated from pydantic model: {{ schema.model_path }}
    """
    return {
{% for rule in schema.rules %}
{% if not rule.condition.startswith('see validator:') and not rule.condition.startswith('/*') %}
        "{{ rule.expectation_name }}": "{{ rule.condition }}",  # {{ rule.rule_type }}: {{ rule.description }}
{% endif %}
{% endfor %}
    }

@dp.table(
    comment="Validated {{ schema.model_name }} records using reusable expectations",
    table_properties={
        "quality": "gold",
...
@dp.expect_all_or_{{ config.validation.default_action }}(get_{{ schema.model_name|snake_case }}_rules())
def {{ schema.model_name|snake_case }}_validated():
    """
    Validated {{ schema.model_name }} records with reusable expectations pattern.
    
    This implementation follows Databricks recommended practices:
    - Stores expectation definitions separately from pipeline logic
    - Enables easy maintenance and updates without modifying pipeline code
    - Supports consistent application across similar datasets
    
    Total validation rules: {{ schema.rules|length }}
    Rule categories:
{% for rule in schema.rules %}
{% if not rule.condition.startswith('see validator:') and not rule.condition.startswith('/*') %}
...
''')
    ...
    def generate_rules_module(self, schemas: Dict[str, ModelValidationSchema]) -> str:
        """
        Generate a separate rules module following Databricks best practices.
        
        This creates a Python module that can be imported and used across multiple pipelines.
        Follows the pattern recommended in Microsoft Databricks documentation.
        """
        template = self.jinja_env.from_string('''"""
Validation Rules Module for Pydantic-Generated DLT Pipelines
===========================================================

Auto-generated from pydantic models on {{ generation_date }}

This module follows Databricks recommended practices for reusable expectations:
- Store expectation definitions separately from pipeline logic
- Easy maintenance and updates without modifying pipeline source code  
- Support for tags to group related expectations
- Consistent application across multiple datasets

Usage:
    from {{ module_name }} import get_rules
    
    @dp.expect_all_or_drop(get_rules('validity'))
    def my_validated_table():
        return dp.read("source_data")
"""

def get_rules_as_list_of_dict():
    """
    Returns all validation rules as a list of dictionaries.
    
    Each rule contains:
    - name: Unique identifier for the expectation
    - constraint: SQL condition that must evaluate to true
    - tag: Category for grouping related expectations
    - model: Source pydantic model name
    - rule_type: Type of validation (field_validator, custom_validator, etc.)
    """
    return [
{% for model_name, schema in schemas.items() %}
        # Rules for {{ model_name }} model
{% for rule in schema.rules %}
{% if not rule.condition.startswith('see validator:') and not rule.condition.startswith('/*') %}
        {
...
def get_rules(tag=None, model=None):
    """
    Returns validation rules as a dictionary, optionally filtered by tag or model.
    
    Args:
        tag: Filter rules by rule type (e.g., 'field_validator', 'custom_validator')
        model: Filter rules by model name (e.g., 'Customer', 'Order')
        
    Returns:
        Dictionary with expectation names as keys and SQL conditions as values
    """
    rules_list = get_rules_as_list_of_dict()
    
    # Apply filters
    if tag:
        rules_list = [rule for rule in rules_list if rule['tag'] == tag]
    if model:
        rules_list = [rule for rule in rules_list if rule['model'] == model]
    
    # Convert to dictionary format expected by @dp.expect_all_or_drop()
    return {
        rule['name']: rule['constraint']
        for rule in rules_list
    }

def get_model_rules(model_name):
    """
    Returns all validation rules for a specific model.
    
    Args:
        model_name: Name of the pydantic model (e.g., 'Customer')
        
    Returns:
        Dictionary with expectation names as keys and SQL conditions as values
    """
    return get_rules(model=model_name)

{% for model_name, schema in schemas.items() %}
def get_{{ model_name|lower }}_rules():
    """
    Returns validation rules specifically for {{ model_name }} model.
    
    Generated from: {{ schema.model_path }}
    Total rules: {{ schema.rules|length }}
    """
    return get_model_rules('{{ model_name }}')

{% endfor %}

# Rule categories available for filtering
RULE_CATEGORIES = [
{% for model_name, schema in schemas.items() %}
{% for rule in schema.rules %}
{% if not rule.condition.startswith('see validator:') and not rule.condition.startswith('/*') %}
...
RULE_CATEGORIES = list(set(RULE_CATEGORIES))  # Remove duplicates
...
@dp.table(comment="{{ schema.model_name }} raw data source")
...
# {{ schema.model_name }} validation rules function
def get_{{ schema.model_name|snake_case }}_validation_rules():
    """
    Returns validation rules for {{ schema.model_name }} following Databricks best practices.
    
    This reusable expectations pattern enables:
    - Easy maintenance and updates without modifying pipeline code  
    - Consistent application across similar datasets
    - External storage and version control of validation logic
...
@dp.table(
    comment="Validated {{ schema.model_name }} records using reusable expectations pattern",
    table_properties={
        "quality": "gold", 
        "pipelines.autoOptimize.managed": "true",
        "validation.source_model": "{{ schema.model_path }}",
...
@dp.expect_all_or_{{ config.validation.default_action }}(get_{{ schema.model_name|snake_case }}_validation_rules())
def {{ schema.model_name|snake_case }}_validated():
    """
    {{ schema.model_name }} records that pass all validation rules.
    
    Uses Databricks recommended reusable expectations pattern for:
    - Improved portability and maintenance
    - Separation of validation logic from pipeline code
    - Consistent rule application across datasets
    """
    return dp.read("{{ schema.model_name|snake_case }}_source")
...
@dp.table(comment="{{ schema.model_name }} records that failed validation")
def {{ schema.model_name|snake_case }}_quarantine():
    """{{ schema.model_name }} records quarantined due to validation failures."""
    base_df = dp.read("{{ schema.model_name|snake_case }}_source")
    
    # Add validation flags
    validated_df = base_df
{% for rule in schema.rules %}
{% if not rule.condition.startswith('see validator:') %}
...
@dp.table(comment="Quarantined {{ schema.model_name }} records")
...
@dp.table(comment="Combined quarantine records from all models")
def quarantine_summary():
    """Combined view of all quarantined records across models."""
    quarantine_dfs = [
{% for schema in schemas.values() %}
        dp.read("quarantine_{{ schema.model_name|snake_case }}"),
{% endfor %}
    ]
...
@dp.table(comment="Data quality metrics summary")
...
{% for schema in schemas.values() %}
    ...
    {{ schema.model_name|snake_case }}_total = dp.read("{{ schema.model_name|snake_case }}_source").count()
    {{ schema.model_name|snake_case }}_valid = dp.read("{{ schema.model_name|snake_case }}_validated").count()
    {{ schema.model_name|snake_case }}_invalid = dp.read("quarantine_{{ schema.model_name|snake_case }}").count()
...
@dp.table(comment="Validation failure analysis")  
def failure_analysis():
    ...
    quarantine_df = dp.read("quarantine_summary")
...
@dp.table(comment="Daily data quality trends")
def daily_quality_trends():
    ...
    metrics_df = dp.read("data_quality_metrics")
